...
@Component({
  selector: 'jhi-referencias-laborales',
...
export class ReferenciasLaboralesComponent implements OnInit, OnDestroy {
  ...
  transition() {
    this.router.navigate(['/referencias-laborales'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  ngOnInit() {
    ...
    this.registerChangeInReferenciasLaborales();
  }
  ...
  trackId(index: number, item: IReferenciasLaborales) {
    return item.id;
  }

  registerChangeInReferenciasLaborales() {
    this.eventSubscriber = this.eventManager.subscribe('referenciasLaboralesListModification', response => this.loadAll());
  }
