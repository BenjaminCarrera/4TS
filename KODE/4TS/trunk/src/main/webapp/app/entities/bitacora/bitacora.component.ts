...
export class BitacoraComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.registerChangeInBitacoras();
  }
  ...
  trackId(index: number, item: IBitacora) {
    return item.id;
  }

  registerChangeInBitacoras() {
    this.eventSubscriber = this.eventManager.subscribe('bitacoraListModification', response => this.loadAll());
  }
