package com.kode.ts.service.dto;

import java.io.Serializable;
import java.util.Objects;
import io.github.jhipster.service.Criteria;
import io.github.jhipster.service.filter.BooleanFilter;
import io.github.jhipster.service.filter.DoubleFilter;
import io.github.jhipster.service.filter.Filter;
import io.github.jhipster.service.filter.FloatFilter;
import io.github.jhipster.service.filter.IntegerFilter;
import io.github.jhipster.service.filter.LongFilter;
import io.github.jhipster.service.filter.StringFilter;

/**
 * Criteria class for the {@link com.kode.ts.domain.ReqCan} entity. This class is used
 * in {@link com.kode.ts.web.rest.ReqCanResource} to receive all the possible filtering options from
 * the Http GET request parameters.
 * For example the following could be a valid request:
 * {@code /req-cans?id.greaterThan=5&attr1.contains=something&attr2.specified=false}
 * As Spring is unable to properly convert the types, unless specific {@link Filter} class are used, we need to use
 * fix type specific filters.
 */
public class ReqCanCriteria implements Serializable, Criteria {

    private static final long serialVersionUID = 1L;

    private LongFilter id;

    private LongFilter candidatoId;

    private LongFilter requerimientoId;

    private LongFilter estatusReqCanId;

    private LongFilter estatusReqCanRecId;

    public ReqCanCriteria(){
    }

    public ReqCanCriteria(ReqCanCriteria other){
        this.id = other.id == null ? null : other.id.copy();
        this.candidatoId = other.candidatoId == null ? null : other.candidatoId.copy();
        this.requerimientoId = other.requerimientoId == null ? null : other.requerimientoId.copy();
        this.estatusReqCanId = other.estatusReqCanId == null ? null : other.estatusReqCanId.copy();
        this.estatusReqCanRecId = other.estatusReqCanRecId == null ? null : other.estatusReqCanRecId.copy();
    }

    @Override
    public ReqCanCriteria copy() {
        return new ReqCanCriteria(this);
    }

    public LongFilter getId() {
        return id;
    }

    public void setId(LongFilter id) {
        this.id = id;
    }

    public LongFilter getCandidatoId() {
        return candidatoId;
    }

    public void setCandidatoId(LongFilter candidatoId) {
        this.candidatoId = candidatoId;
    }

    public LongFilter getRequerimientoId() {
        return requerimientoId;
    }

    public void setRequerimientoId(LongFilter requerimientoId) {
        this.requerimientoId = requerimientoId;
    }

    public LongFilter getEstatusReqCanId() {
        return estatusReqCanId;
    }

    public void setEstatusReqCanId(LongFilter estatusReqCanId) {
        this.estatusReqCanId = estatusReqCanId;
    }

    public LongFilter getEstatusReqCanRecId() {
        return estatusReqCanRecId;
    }

    public void setEstatusReqCanRecId(LongFilter estatusReqCanRecId) {
        this.estatusReqCanRecId = estatusReqCanRecId;
    }


    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        final ReqCanCriteria that = (ReqCanCriteria) o;
        return
            Objects.equals(id, that.id) &&
            Objects.equals(candidatoId, that.candidatoId) &&
            Objects.equals(requerimientoId, that.requerimientoId) &&
            Objects.equals(estatusReqCanId, that.estatusReqCanId) &&
            Objects.equals(estatusReqCanRecId, that.estatusReqCanRecId);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
        id,
        candidatoId,
        requerimientoId,
        estatusReqCanId,
        estatusReqCanRecId
        );
    }

    @Override
    public String toString() {
        return "ReqCanCriteria{" +
                (id != null ? "id=" + id + ", " : "") +
                (candidatoId != null ? "candidatoId=" + candidatoId + ", " : "") +
                (requerimientoId != null ? "requerimientoId=" + requerimientoId + ", " : "") +
                (estatusReqCanId != null ? "estatusReqCanId=" + estatusReqCanId + ", " : "") +
                (estatusReqCanRecId != null ? "estatusReqCanRecId=" + estatusReqCanRecId + ", " : "") +
            "}";
    }

}
