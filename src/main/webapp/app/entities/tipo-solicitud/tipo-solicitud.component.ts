...
export class TipoSolicitudComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.tipoSolicitudService
      ...
      .subscribe(
        (res: HttpResponse<ITipoSolicitud[]>) => this.paginateTipoSolicituds(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/tipo-solicitud'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  protected paginateTipoSolicituds(data: ITipoSolicitud[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.tipoSolicituds = data;
  }
