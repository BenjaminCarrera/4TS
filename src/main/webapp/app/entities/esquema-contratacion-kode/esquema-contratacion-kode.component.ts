...
@Component({
  selector: 'jhi-esquema-contratacion-kode',
  templateUrl: './esquema-contratacion-kode.component.html'
})
export class EsquemaContratacionKodeComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  transition() {
    this.router.navigate(['/esquema-contratacion-kode'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  ngOnInit() {
    ...
    this.registerChangeInEsquemaContratacionKodes();
  }
  ...
  trackId(index: number, item: IEsquemaContratacionKode) {
    return item.id;
  }

  registerChangeInEsquemaContratacionKodes() {
    this.eventSubscriber = this.eventManager.subscribe('esquemaContratacionKodeListModification', response => this.loadAll());
  }
