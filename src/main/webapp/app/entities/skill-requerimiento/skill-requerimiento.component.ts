...
@Component({
  selector: 'jhi-skill-requerimiento',
  templateUrl: './skill-requerimiento.component.html'
})
export class SkillRequerimientoComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  transition() {
    this.router.navigate(['/skill-requerimiento'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  ngOnInit() {
    ...
    this.registerChangeInSkillRequerimientos();
  }
  ...
  trackId(index: number, item: ISkillRequerimiento) {
    return item.id;
  }

  registerChangeInSkillRequerimientos() {
    this.eventSubscriber = this.eventManager.subscribe('skillRequerimientoListModification', response => this.loadAll());
  }
